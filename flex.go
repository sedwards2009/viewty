...
func (f *Flex) Reposition(x, y, width, height int) {
	f.WidgetBase.Reposition(x, y, width, height)

	if f.vertical {
		yList, heightList := layout(height, f.gapSize, f.items)
		for i, item := range f.items {
			item.widget.Reposition(x, y + yList[i], width, heightList[i])
		}
	} else {
		xList, widthList := layout(width, f.gapSize, f.items)
		for i, item := range f.items {
			item.widget.Reposition(x +xList[i], y, widthList[i], height)
		}
	}
}
...
func (f *Flex) Render(screen tcell.Screen) {
	for _, item := range f.items {
		item.widget.Render(screen)
	}
}

func (f *Flex) ChildWidgetAt(x int, y int) Widget {
	for _, item := range f.items {
		if item.widget.ContainsPoint(x, y) {
			childWidget := item.widget.ChildWidgetAt(x, y)
			if childWidget != nil {
				return childWidget
			}
			return item.widget
